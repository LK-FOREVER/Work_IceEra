...
export class staff_view extends Component {
    ...
    initStaff() {
        for (let i = 0; i < this.staff_config.length; i++) {
            ...
            //头像
            Utils.getSpriteFrame("ui/staff_head/" + this.staff_config[i].icon_id, staff_item.getChildByName("staff_icon").getComponent(Sprite));
            //名称
            ...
            lv_txt.string = "等级：" + GameData.userDataProxy.staffLv[this.staff_config[i].id];
        }
    }
    get_staff_config(): Promise<void> {
        return new Promise((resolve, reject) => {
            const path = "config/data/adventure__get_staff_conf"
